...
class CodaReader_old {
  ...
  private function log($msg) {
    $class = '';
    if ($msg[0] != ' ') $class .= ' bb';
    if (preg_match('/WARNING/',$msg)) $class .= ' red';
    $this->log[] = array( 'cls' => $class, 'msg' => $msg);
  }
...
//		print_r($this->codabatches);
  ...
  protected function processRecord1($process_record) {
    ...
    switch (substr($process_record, 1, 1)) {
      case 0:
        ...
        $this->log('BBAN : '.$this->codabatch->bban);
      ...
      case 2:
        ...
        $this->log('IBAN : '.$this->codabatch->iban);
      ...
      case 3:
        ...
        $this->log('NBAN : '.$this->codabatch->bban);
        //currency code
        $this->codabatch->currency = $this->filterWhiteSpace(substr($process_record, 39, 3));
    ...
    $this->codabatch->sequence = substr($process_record, 2, 3);
    $this->codabatch->source .= $process_record . '/\n/';

    $this->log('  statement # : ' . $this->codabatch->sequence);
    $this->log('  starting balance : '.number_format($this->codabatch->starting_balance,2,'.','') . ' &euro; on ' . $this->codabatch->starting_date);
  ...
  protected function processRecord8($process_record) {
    ...
    $this->log('  ending balance : '.number_format($this->codabatch->ending_balance,2,'.','') . ' &euro; on ' . $this->codabatch->ending_date);
  ...
  protected function processRecord9($process_record) {
    ...
    $this->log('  total movements : ' . 1*$nrrecs);
    $this->log('  total transactions : ' . count($this->codarecords));
